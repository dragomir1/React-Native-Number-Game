...
// importing icons from expo to use.
...
// we add the function outside of the component becuase we dont end anything from this function, it doesn't need to be part of the re-render.

// 'numOfRound' is an extra argument and needs to be used as the first argument.  RENDERITEM in FLATLIST only forwards one argument...we are binding multitple arguments that should be passed in...look down in FLATLIST comp.

// we can get the index out of the 'itemData'.  It's the default argument passed by react as the last argument.
// the first argument is the numOfRound that was set up with bind in the flatlist comp. and any default arguments which would have been passed down normally, will be added as aditional arguments at the end of the argument list.
const renderItemsInList = (listLength, itemData) => (
  <View style={styles.listViewContainer}>
    {/* calculating the number of rounds. itemData is given automatically by RN is an object that has an 'index' prop , which is the index of the item being outputted.*/}
    <FontBodyText>#{listLength - itemData.index}</FontBodyText>
    {/* 'itemData.item' is the item itself which in this guess is the guess number*/}
    <FontBodyText>{itemData.item}</FontBodyText>
  </View>
);
...
const GameScreen = props => {
  ...
  const initialGuess = generateRandomNum(1, 100, props.userChoiceNum);
  const [currentGuess, setCurrentGuessState] = useState(initialGuess);

  //this state manages the round it took to guess the number.  WE are settign up to register and save the past guess of the computer to be outputted as a list.  we are setting an initial guess.
  // WE ARE SETTING THE INITAL VALUE AS A STRING AND CREATED AN ARRAY OF VALUES THAT ARE STRING.  REASON IS TO USE THESE VALUES AS KEYS TO BE EXTRACTED IN FLATLIST COMP.  IT EXPECTS A STRING AS A RETURN VALUE.
  ...
  useEffect(() => {
    ...
    if (currentGuess === userChoiceNum) {
      ...
      // we are forwarding a number. but pastGuesses is an array and we cant do that. so we just forward the length of that array. which is essentially the number of rounds
  ...
  const nextGuessHandler = direction => {
    ...
    // USING NEXTNUMBER AS A KEY FOR OUR FLATLIST. IT EXPECTS A STRING SO WE NEED TO CONVERT THE NUMBER TO STRINGS..THESE VALUES ARE ONLY USED FOR PRINTING TO SCREEN. SO IT'S NO ISSUE.
      ...
      <Card style={styles.buttonStyling}>
        ...
        <View>
          <CustomButton
            /* the value we have here passed as an argument should differ based on the button.  need to preconfigure the argument that's passed to the handler when it's executed.  we use the bind method for preconfiguration.*/
            // "this" is the first argument to bind. its what's should be referred to in the function thats getting called.
            // 'higher' is the first agrument recieved by the handler
            onPress={nextGuessHandler.bind(this, 'higher')}>
            <Ionicons name='md-add' size={24} color='white' />
          </CustomButton>
        </View>
      </Card>
      {/* to set desired width and height wrap the compenent in a view and set width/height in that view */}
      <View style={styles.listContainer}>
        {/* this takes two important props. data is data to display. renderItem allows you to output the data you're feeding in.  RENDER ITEM IN FLATLIST COMP ONLY FOWARDS ONE ITEM => THE DATA. we also need to provide a key.  
        FLATLISTS EXPECTS TO GET AN AN OBJECT IN THE ARRAY IF ITEMS WE'RE PASSING IN WHERE EACH OBJECT HAS A KEY PROPERTY THAT CAN BE EXTRACTED AND USED WHEN RENDERING
        Key Extractor takes a funciton. AND WE TELL FLAT LIST WHERE TO FIND THE KEY.  BY DEFAULT IT LOOKS FOR 'ITEM.KEY' OR 'ITEM.ID'.  HERE WE TELL IT THE ITEM ITESELF IS THE KEY. FLATLIST KEYS NEED TO BE STRINGS AND NOT NUMBERS.
      */}
        <FlatList
          data={pastGuesses}
          // EXPECTS A STRING AS A VALUE. NEED TO CONVERT NUMS TO STRS IN THE GUESSES ARRAY.
          keyExtractor={item => item}
          // we are binding arguments that should be passed in so we can add additional arguments to the function besides the default argument.
          // .bind() expects 'this' as the fist argument. we set what this should refer to in the function being called.
          // passing the length of the arguments as second agrument. these are string values. we dont' have access to index anymore so we're passing the length of the array.
          // PASTGUESS.LENGHT IS ONLY ONE ARGUMENT ON THE RENDERITMENSLIST FUNCTION - THIS REPRESENTS THE FIRST ARGUMENT IN THE FUCTION. THE SECOND ARGUMENT IS THE DEFAULT ARGUMENT THAT WOULD'VE BEEN PASSED DONW NORMALLY.
          renderItem={renderItemsInList.bind(this, pastGuesses.length)}
...
const styles = StyleSheet.create({
  ...
  listContainer: {
    // we added flex to make the list scrollable on andriod
    flex: 1,
    width: '60%'
  },
  listViewContainer: {
    ...
    flexDirection: 'row',
    // relfects styling along main axis - in this case 'row'
    justifyContent: 'space-around',
    width: '100%'
  },
  list: {
    // to set desired width and height wrap the compenent in a view and set width/height in that view/
    // this is used with scrollview comp.flexgrow is able to grow and able to take as much space as available as well as exceed the bourndries of the screen as well.
    flexGrow: 1,
